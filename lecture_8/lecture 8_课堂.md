...
### 定义
存储过程是在大型数据库系统中，一组为了完成特定功能的SQL语句集，存储在数据库中，经过第一次编译后再次调用不需要再次编译，用户通过指定存储过程的名字并给出参数（如果该存储过程带有参数）来执行它。
### 适用场景
存储过程适用于处理较为复杂的业务场景：
- 对响应时间有要求：在前台处理可能会涉及到多次数据库连接，但用存储过程就只有一次，存储过程可以提高运行效率；
- 对稳定性和安全性有要求：使用了存储过程的系统更加稳定，只要数据库不出现问题，基本上不会出现问题。
...
参考资料：https://www.cnblogs.com/story-xc/p/10997088.html
